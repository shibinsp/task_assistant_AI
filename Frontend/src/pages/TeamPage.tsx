...
import { mapApiUserToTeamMember, type FrontendTeamMember } from '@/types/mappers';
...
function InviteDialog({ children }: { children: React.ReactNode }) {
  ...
  const inviteMutation = useMutation({
    mutationFn: () => teamService.createUser({
      email,
      password: 'TempPass123!',
      first_name: email.split('@')[0],
      last_name: 'User',
      role: (selectedRole ?? 'employee') as 'employee',
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.team.users() });
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-lg border transition-colors text-left ${
                      selectedRole === key ? 'border-primary bg-primary/5' : 'border-border/50 hover:border-primary/50 hover:bg-primary/5'
                    }`}
                  >
...
export default function TeamPage() {
  ...
  const teamMembers: FrontendTeamMember[] = (usersData?.users ?? []).map(mapApiUserToTeamMember);

  // If no API data yet, fall back to mock data
  ...
  const activeMembers = displayTeam.filter((m) => m.status === 'active').length;
  const totalTasks = usersData?.total ?? displayTeam.length;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Members', value: usersData?.total ?? displayTeam.length, icon: User },
            { label: 'Active Now', value: activeMembers, icon: CheckCircle2 },
            { label: 'Total Users', value: totalTasks.toLocaleString(), icon: CheckCircle2 },
            { label: 'Avg Efficiency', value: avgEfficiency ? `${avgEfficiency}%` : 'N/A', icon: Shield },
          ].map((stat, index) => {
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredTeam.map((member) => {
                      ...
                      const tasksCompleted = 'tasksCompleted' in member ? (member as TeamMember).tasksCompleted : 0;
                      const tasksInProgress = 'tasksInProgress' in member ? (member as TeamMember).tasksInProgress : 0;
                      const efficiency = 'efficiency' in member ? (member as TeamMember).efficiency : 0;
                        ...
                        <TableRow key={member.id}>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              ...
                              <span className="text-muted-foreground"> / {tasksInProgress} in progress</span>
                            </div>
                          </TableCell>
                          <TableCell>
