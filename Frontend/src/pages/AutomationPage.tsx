...
export default function AutomationPage() {
  ...
  const { data: roi } = useQuery({
    queryKey: queryKeys.automation.roi,
  ...
  // Map agents to workflow format for existing UI
  const workflows: Workflow[] = (agents ?? []).map((agent) => ({
    id: agent.id,
    name: agent.name,
    ...
    lastRun: agent.last_run_at
      ...
      : 'Never',
    runsCount: agent.total_runs,
    successRate: agent.total_runs > 0
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Active Workflows', value: roi?.active_agents?.toString() ?? '0', icon: Play, color: 'text-emerald-500' },
            { label: 'Total Agents', value: roi?.total_agents?.toString() ?? '0', icon: Zap, color: 'text-primary' },
            { label: 'Patterns Detected', value: roi?.patterns_detected?.toString() ?? '0', icon: CheckCircle2, color: 'text-blue-500' },
            { label: 'Time Saved', value: roi?.total_hours_saved ? `${Math.round(roi.total_hours_saved)}h` : '0h', icon: Clock, color: 'text-amber-500' },
          ].map((stat, index) => {
