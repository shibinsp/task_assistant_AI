...
async def list_agents(
    ...
    for name, agent_info in stats.get("agents", {}).items():
        ...
        agents.append(AgentInfo(
            ...
            success_rate=_calculate_success_rate(agent_info),
            last_execution_at=_parse_datetime(agent_info.get("last_execution")),
        ))
...
async def get_agent(
    ...
    return AgentInfo(
        ...
        last_execution_at=_parse_datetime(stats.get("last_execution")),
    )
...
def _calculate_success_rate(stats: dict) -> float:
    ...
    return round((execution_count - error_count) / execution_count * 100, 2)
